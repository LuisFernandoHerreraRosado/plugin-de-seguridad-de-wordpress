...
class WPSM_Firewall {
    ...
    private function check_user_agent() {
        ...
        $bad_uas = array_map( 'trim', explode( "\n", $this->settings->get_setting( 'firewall_bad_uas', '' ) ) );
    ...
    private function check_custom_rules() {
        ...
        foreach ( $custom_rules as $rule ) {
            ...
            switch ( $rule['type'] ) {
                case 'url':
                    $match = preg_match( '/' . str_replace( '/', '\/', $rule['pattern'] ) . '/i', $request_uri );
                    break;
                case 'ua':
                    $match = preg_match( '/' . str_replace( '/', '\/', $rule['pattern'] ) . '/i', $ua );
                    break;
                case 'param':
                    $match = preg_match( '/' . str_replace( '/', '\/', $rule['pattern'] ) . '/i', $query_string );
                    break;
            }
    ...
    private function check_referrer() {
        ...
        $bad_referrers = array_map( 'trim', explode( "\n", $this->settings->get_setting( 'firewall_bad_referrers', '' ) ) );
